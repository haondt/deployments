...
class FlaskApp:
    def __init__(self, on_ping, ping_key):
        self.app = Flask(__name__)
        @self.app.route(f'/{ping_key}/gabbro/ping', methods=['GET'])
        def ping():
            on_ping()
            return 'OK', 200
...
def main():
    ...
    ping_key = os.getenv('PING_KEY')
    if (discord_webhook_url is None or ping_timeout_ms is None or ping_key is None):
    ...
    app = FlaskApp(lambda: state.ping(), ping_key)
